...
//  NVIC_SetPriorityGrouping(0);
  ...
  xQueue = xQueueCreate(5,sizeof(LedState_t*));
...
void vTarea1(void *params){
	...
	for(;;){
		...
		if (HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13)==0) {
			data->led = 1;
			data->sw = 1;
			strcpy((char*)data->str,"Led Encendido\r\n");
		}else{
			data->led =0;
			data->sw = 0;
			strcpy((char*)data->str,"Led APAGADO\r\n");
		}
		data->msDelay = 100;

		if (xQueueSend(xQueue,&data,portMAX_DELAY)==pdTRUE) {
			SEGGER_SYSVIEW_PrintfHost("dato enviado\r\r");
			//SendUart("dato enviado\r\n");
		}

		vTaskDelay(pdMS_TO_TICKS(data->msDelay));
	}
	vTaskDelete(NULL);
}
...
void vTarea2(void *params){
	...
	for(;;){
		if (xQueueReceive(xQueue, &data, portMAX_DELAY)==pdTRUE) {
					SEGGER_SYSVIEW_PrintfHost("dato recibido\r\r");
					SendUart("Estado Pulsador->%d\r\n",data->sw);
					SendUart("%s\n\r",data->str);
					HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5,data->led);
				}
	}
	vTaskDelete(NULL);
}
